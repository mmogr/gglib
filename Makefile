...
uninstall:
	@echo "⚠️  WARNING: This will uninstall gglib and remove system data:"
	@echo "  - Binary from ~/.cargo/bin"
	@echo "  - System configuration and database (~/Library/Application Support/gglib or ~/.local/share/gglib)"
	@echo "  - Local build artifacts and untracked files (via git clean)"
...
llama-install:
	...
	@if command -v gglib >/dev/null 2>&1; then gglib llama install; else cargo run -- llama install; fi
...
llama-update:
	...
	@if command -v gglib >/dev/null 2>&1; then gglib llama update; else cargo run -- llama update; fi

llama-status:
	@if command -v gglib >/dev/null 2>&1; then gglib llama status; else cargo run -- llama status; fi
...
run-serve:
	...
	cargo run --release -- serve $(if $(ID),$(ID),1)
...
run-web:
	...
	cargo run -- web $(if $(PORT),--port $(PORT),)
...
setup: check-deps build-gui build-tauri install
	...
	@cargo run -- config models-dir prompt
