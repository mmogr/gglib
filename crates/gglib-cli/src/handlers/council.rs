//! CLI handler for `gglib council` — plan and execute a Director/Worker
...
async fn render_event(
    event: &CouncilEvent,
    approval_registry: &Arc<gglib_app_services::CouncilApprovalRegistry>,
) {
    match event {
...
async fn prompt_and_resolve(
    approval_id: &str,
    kind: &ApprovalKind,
    registry: &Arc<gglib_app_services::CouncilApprovalRegistry>,
) {
