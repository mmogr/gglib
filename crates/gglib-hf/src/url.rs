...
pub(crate) fn build_expand_params() -> String {
    EXPAND_FIELDS
        .iter()
...
pub(crate) fn build_tree_url(config: &HfConfig, repo: &HfRepoRef, path: Option<&str>) -> Url {
    ...
    let tree_path = match path {
        Some(p) => format!("{}/tree/main/{p}", repo.id()),
...
pub(crate) fn build_download_url(repo: &HfRepoRef, file_path: &str, revision: Option<&str>) -> Url {
