...
///
/// ```rust
/// use gglib_agent::orchestrator::director::DirectorPlan;
/// use serde_json::json;
///
/// let raw = json!({
///     "goal": "example",
...
/// - `tx` — Optional event channel; when `Some`, emits [`OrchestratorEvent`]s
...
///
/// ```rust
/// use gglib_agent::orchestrator::director::{DirectorNode, DirectorPlan};
/// use gglib_core::domain::orchestrator::task_graph::{NodeId, TaskNode, NodeStatus};
///
    ...
    for attempt in 0..total_attempts {
        if attempt > 0 {
            ...
            if let Some(ref tx) = tx {
                let _ = tx
                    .send(OrchestratorEvent::ReplanAttempt {
                        attempt,
