//! Unit tests for [`gglib_agent::collect_stream`] and [`gglib_agent::CollectedResponse`].
//!
//! Tests lived in `src/stream_collector.rs` until they were extracted here to
//! keep production source files small and to share the common test-infra path.
//! The duplicate `missing_done_event_returns_error` test that was previously
//! present alongside `truncated_stream_returns_distinct_error` has been removed
//! — it was character-for-character identical.
