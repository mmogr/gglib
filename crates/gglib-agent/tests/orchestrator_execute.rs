...
//!   sees no tools.
//! - Fail-fast: a worker error causes `OrchestratorError` on the stream and
//!   `ExecuteError` from `execute()`.
//! - Happy path: `OrchestratorComplete` is the last event on success.
//!
//! # Stub LLM design
//!
...
/// Collect all events from an `OrchestratorEvent` channel into a `Vec`.
...
/// Happy path: single node, all events arrive, `OrchestratorComplete` is last.
#[tokio::test]
async fn single_node_happy_path_ends_with_complete() {
    ...
    assert!(
        has_complete,
        "must have OrchestratorComplete; got: {events:?}"
    );

    // NodeStarted for 'answer' must appear.
    let has_node_started = events.iter().any(
        |e| matches!(e, OrchestratorEvent::NodeStarted { node_id, .. } if node_id == "answer"),
    );
...
async fn two_node_topological_order() {
    ...
    let research_started = events.iter().position(
        |e| matches!(e, OrchestratorEvent::NodeStarted { node_id, .. } if node_id == "research"),
    );
    let research_complete = events.iter().position(
        |e| matches!(e, OrchestratorEvent::NodeComplete { node_id, .. } if node_id == "research"),
    );
    let write_started = events.iter().position(
        |e| matches!(e, OrchestratorEvent::NodeStarted { node_id, .. } if node_id == "write"),
    );
...
/// Worker error causes `OrchestratorError` event and `ExecuteError` return.
#[tokio::test]
async fn worker_error_triggers_fail_fast() {
    ...
    // Must emit OrchestratorError.
    let has_error = events
        .iter()
        .any(|e| matches!(e, OrchestratorEvent::OrchestratorError { .. }));
    assert!(has_error, "must have OrchestratorError; got: {events:?}");
...
async fn plan_approved_event_emitted() {
    ...
    let proposed_pos = events
        .iter()
        .position(|e| matches!(e, OrchestratorEvent::PlanProposed { .. }));
    let approved_pos = events
        .iter()
        .position(|e| matches!(e, OrchestratorEvent::PlanApproved));
...
async fn synthesis_events_emitted() {
    ...
    let synth_start = events
        .iter()
        .position(|e| matches!(e, OrchestratorEvent::SynthesisStart));
    let synth_complete = events
        .iter()
        .position(|e| matches!(e, OrchestratorEvent::SynthesisComplete { .. }));
