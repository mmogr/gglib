...
//! the real [`CouncilApprovalRegistry`] and in-memory
//! [`SqliteCouncilRepository`] to exercise the gate logic end-to-end.
//!
//! # What is tested
//!
...
//!    continues to `CouncilComplete` after the registry is resolved with
//!    `Approve`.
//!
...
async fn make_repo() -> Arc<SqliteCouncilRepository> {
    let pool = setup_test_database().await.unwrap();
    Arc::new(SqliteCouncilRepository::new(pool))
}
...
/// Gate: `ApprovePlan` — approve → run completes with `CouncilComplete`.
#[tokio::test]
async fn hitl_approve_plan_continues_execution() {
    ...
    let approval_id = loop {
        if let Ok(event) = rx.try_recv() {
            if let CouncilEvent::AwaitingApproval { approval_id, .. } = event {
                break approval_id;
            }
        } else {
    ...
    let has_plan_approved = remaining
        .iter()
        .any(|e| matches!(e, CouncilEvent::PlanApproved));
    assert!(
        has_plan_approved,
...
async fn hitl_reject_plan_aborts_run() {
    ...
    let has_rejected = remaining
        .iter()
        .any(|e| matches!(e, CouncilEvent::PlanRejected { .. }));
    assert!(
        has_rejected,
...
async fn interrupted_runs_marked_on_startup() {
    ...
    let make_run = |id: &str, status: CouncilRunStatus| CouncilRun {
        id: id.to_string(),
    ...
    repo.create_run(make_run("r1", CouncilRunStatus::Running))
        .await
        .unwrap();
    repo.create_run(make_run("r2", CouncilRunStatus::Running))
        .await
        .unwrap();
    repo.create_run(make_run("r3", CouncilRunStatus::Completed))
        .await
        .unwrap();
    ...
    assert!(
        matches!(r1.status, CouncilRunStatus::Interrupted),
        "r1 should be Interrupted"
    );
    assert!(
        matches!(r2.status, CouncilRunStatus::Interrupted),
        "r2 should be Interrupted"
    );
    assert!(
        matches!(r3.status, CouncilRunStatus::Completed),
...
async fn hitl_none_mode_auto_approves() {
    ...
    let result = execute(
        ...
        Arc::new(llm),
        tool_executor,
        CouncilConfig::default(), // HitlMode::None
        tx,
    )
    .await;
    ...
    assert!(
        !events
            .iter()
            .any(|e| matches!(e, CouncilEvent::AwaitingApproval { .. })),
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, CouncilEvent::CouncilComplete { .. })),
        "CouncilComplete must appear; got: {events:?}"
    );
}
