...
impl ProcessCore {
    ...
    pub fn spawn(&mut self, config: &ServerConfig) -> Result<ProcessHandle> {
        ...
        let mut child = command::build_and_spawn(&self.llama_server_path, config, port)?;
        let pid = child.id();

        command::spawn_log_readers(&mut child, port);
    ...
    pub fn kill(&mut self, model_id: i64) -> Result<()> {
        let running = self
            .processes
            .remove(&model_id)
        ...
        debug!(model_id = %model_id, pid = %pid, "Killing process");
    ...
    pub fn kill_all(&mut self) {
        let ids: Vec<i64> = self.processes.keys().copied().collect();
        for id in ids {
            let _ = self.kill(id);
        }
    }
    ...
    pub fn cleanup_dead(&mut self) -> Vec<i64> {
        let dead: Vec<i64> = self
            .processes
            .iter_mut()
            .filter_map(|(id, r)| match r.child.try_wait() {
                Ok(Some(_)) | Err(_) => Some(*id),
                Ok(None) => None,
            })
            .collect();
