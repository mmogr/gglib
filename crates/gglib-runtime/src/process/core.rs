...
impl GuiProcessCore {
    ...
    fn spawn_log_readers(&self, child: &mut std::process::Child, port: u16) {
        if let Some(stdout) = child.stdout.take() {
            ...
            std::thread::spawn(move || {
                ...
                for line in reader.lines() {
                    match line {
                        Ok(text) => log_manager.add_log(log_port, &text),
                        Err(e) => {
    ...
    pub fn kill(&mut self, model_id: u32) -> Result<()> {
        #[cfg(unix)]
        {
            if let Some(running) = self.processes.remove(&model_id) {
                ...
                debug!(model_id = %model_id, pid = %pid, port = %running.info.port, "Killing process");
                ...
                drop(running);
                debug!("SIGKILL sent to process {}, handle dropped", pid);
                Ok(())
            } else {
