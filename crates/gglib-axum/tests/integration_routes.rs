...
// ============================================================================
// Proxy API Tests (PR1 - disabled during Phase 2 refactor #221)
// ============================================================================

#[tokio::test]
async fn proxy_status_returns_disabled_state() {
    let ctx = match bootstrap(test_config()).await {
        Ok(ctx) => ctx,
        Err(_) => return,
    };
    ...
    // Phase 2 behavior: proxy disabled, running == false
    let body = response.into_body().collect().await.unwrap().to_bytes();
    let body_str = std::str::from_utf8(&body).unwrap();
    assert!(
        body_str.contains("\"running\":false"),
        "Proxy should report running:false during Phase 2, got: {}",
        body_str
    );
}

#[tokio::test]
async fn proxy_start_returns_service_unavailable_during_phase2() {
    let ctx = match bootstrap(test_config()).await {
        Ok(ctx) => ctx,
        Err(_) => return,
    };
    ...
    // Contract: 503 Service Unavailable with JSON error (proxy disabled during Phase 2 #221)
...
#[tokio::test]
async fn proxy_stop_returns_service_unavailable_during_phase2() {
    let ctx = match bootstrap(test_config()).await {
        Ok(ctx) => ctx,
        Err(_) => return,
    };
