//! `POST /api/orchestrator/runs/{id}/resume` — resume a parked run.
//!
...
/// Request body for `POST /api/orchestrator/runs/{id}/resume`.
...
pub async fn resume_run(
    ...
    match run.status {
        OrchestratorRunStatus::Completed | OrchestratorRunStatus::Failed => {
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            tracing::error!(error = %e, run_id, "orchestrator: resume failed");
        }
    });
