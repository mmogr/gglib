//! `POST /api/council/run` — execute a full Director/Worker task graph.
//!
...
//! [`CouncilEvent`]s as newline-delimited JSON SSE frames.
//!
//! # Event sequence
//!
//! 1. Zero or more [`CouncilEvent::ReplanAttempt`] events.
//! 2. [`CouncilEvent::PlanProposed`] containing the validated graph.
//! 3. [`CouncilEvent::PlanApproved`] (auto-approved in Phase C).
...
//! 6. [`CouncilEvent::CouncilComplete`] with the final answer.
//!
//! On failure: [`CouncilEvent::NodeFailed`] for the failed node, then
//! [`CouncilEvent::CouncilError`] and the stream closes.
...
/// Stream a full orchestrator run as [`CouncilEvent`] SSE frames.
///
/// # Errors
///
...
pub async fn run_sse(
    ...
    // we must do it here.
    {
        ...
        if let Err(e) = state.council_repo.create_run(run).await {
            tracing::warn!(run_id, error = %e, "council: failed to pre-create run record");
        }
    }
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            // execute() already sent CouncilError on the channel before
            // returning Err, so we only log here for server-side observability.
            tracing::error!(error = %e, goal, "council: run failed");
        }
