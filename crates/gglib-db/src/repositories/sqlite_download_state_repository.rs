...
use gglib_core::{
    DownloadId, DownloadStateRepositoryPort, DownloadStatus, QueuedDownload, Quantization,
...
fn row_to_queued_download(row: &sqlx::sqlite::SqliteRow) -> Result<QueuedDownload, RepositoryError> {
    use sqlx::Row;
    ...
    let shard_info: Option<ShardInfo> = shard_info_json
