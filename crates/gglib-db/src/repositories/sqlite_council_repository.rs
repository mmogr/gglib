//! `SQLite` implementation of the [`CouncilRepositoryPort`] trait.
...
fn status_to_str(status: &CouncilRunStatus) -> &'static str {
    match status {
        CouncilRunStatus::Running => "running",
        CouncilRunStatus::AwaitingApproval => "awaiting_approval",
        CouncilRunStatus::Interrupted => "interrupted",
        CouncilRunStatus::Completed => "completed",
        CouncilRunStatus::Failed => "failed",
    }
}

fn str_to_status(s: &str) -> CouncilRunStatus {
    match s {
        "awaiting_approval" => CouncilRunStatus::AwaitingApproval,
        "interrupted" => CouncilRunStatus::Interrupted,
        "completed" => CouncilRunStatus::Completed,
        "failed" => CouncilRunStatus::Failed,
        _ => CouncilRunStatus::Running,
    }
}
...
#[async_trait]
impl CouncilRepositoryPort for SqliteCouncilRepository {
...
mod tests {
    ...
    async fn make_repo() -> SqliteCouncilRepository {
        let pool = setup_test_database().await.unwrap();
        SqliteCouncilRepository::new(pool)
    }

    fn make_run(id: &str) -> CouncilRun {
        CouncilRun {
            id: id.to_string(),
    ...
    async fn create_and_get_run() {
        ...
        assert!(matches!(got.status, CouncilRunStatus::Running));
    }
    ...
    async fn update_run_status() {
        ...
        repo.update_run_status("run-2", CouncilRunStatus::Completed)
            .await
            .unwrap();
        let got = repo.get_run("run-2").await.unwrap().unwrap();
        assert!(matches!(got.status, CouncilRunStatus::Completed));
    }
    ...
    async fn append_and_list_events() {
        ...
        for i in 0..3i64 {
            repo.append_event(CouncilRunEvent {
    ...
    async fn mark_interrupted_runs() {
        ...
        repo.update_run_status("run-5", CouncilRunStatus::Completed)
            .await
            .unwrap();
        ...
        assert!(matches!(r4.status, CouncilRunStatus::Interrupted));
        let r5 = repo.get_run("run-5").await.unwrap().unwrap();
        assert!(matches!(r5.status, CouncilRunStatus::Completed));
    }
    ...
    async fn list_runs_with_filter() {
        ...
        repo.update_run_status("run-7", CouncilRunStatus::Failed)
            .await
            .unwrap();
        ...
        let failed = repo
            .list_runs(Some(CouncilRunStatus::Failed))
            .await
            .unwrap();
        assert!(
            failed
                .iter()
                .all(|r| matches!(r.status, CouncilRunStatus::Failed))
        );
