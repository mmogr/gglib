...
impl QueuedItem {
    ...
    pub fn to_dto(&self, position: u32, status: DownloadStatus) -> gglib_core::download::QueuedDownload {
        ...
        dto.status = status;
        dto.group_id = self.group_id.as_ref().map(|g| g.to_string());
        dto.shard_info = self.shard_info.clone();
...
impl FailedItem {
    ...
    pub fn to_dto(&self) -> gglib_core::download::FailedDownload {
        let display_name = if let Some(ref shard) = self.item.shard_info {
            format!("{} ({})", self.item.id, shard.display())
        } else {
...
mod tests {
    ...
    fn test_queued_item_shard() {
        ...
        let item = QueuedItem::new_shard(id.clone(), group_id.clone(), shard_info);
