...
use gglib_core::download::{DownloadError, DownloadId, DownloadStatus, QueueSnapshot, ShardInfo};

pub use shard_group::ShardGroupId;
pub use types::{FailedItem, QueueCommand, QueueEvent, QueuedItem};
...
impl DownloadQueue {
    ...
    pub fn retry_failed(
        ...
        // Add back to pending queue with fresh timestamp
        let item = QueuedItem::new(failed.item.id);
        self.pending.push_back(item);
...
mod tests {
    ...
    fn test_remove_pending_item() {
        ...
        queue.queue(id_a.clone(), false).unwrap();
        queue.queue(id_b.clone(), false).unwrap();
    ...
    fn test_reorder_to_front() {
        ...
        queue.queue(test_id("a", None), false).unwrap();
        queue.queue(test_id("b", None), false).unwrap();
        queue.queue(test_id("c", None), false).unwrap();
    ...
    fn test_retry_failed_with_existing_queue() {
        let mut queue = DownloadQueue::new(10);
        queue.queue(test_id("existing", None), false).unwrap();
    ...
    fn test_clear_failed() {
        let mut queue = DownloadQueue::new(10);
        queue.mark_failed(QueuedItem::new(test_id("a", None)), "err1");
        queue.mark_failed(QueuedItem::new(test_id("b", None)), "err2");
