...
impl DownloadManagerImpl {
    ...
    async fn run_loop(&self) {
        loop {
            ...
            if let Some((lease, item, cancel, progress_tx)) = self.next_job().await {
                ...
                let deps = WorkerDeps {
                    model_registrar: Arc::clone(&self.model_registrar),
                ...
                // Finalize the job
                self.finalize_job(&item.id, lease, result).await;
    ...
    async fn finalize_job(
        ...
        if !self.verify_and_remove_lease(id, lease).await {
            tracing::debug!(id = %id, "Ignoring stale finalize (lease mismatch)");
            return;
        }

        // Handle the result
        self.handle_job_result(id, result).await;
    ...
    async fn handle_job_result(
        ...
        match result {
            Ok(completed) => {
                tracing::info!(id = %id, "Download completed");
                self.event_emitter.emit(DownloadEvent::DownloadCompleted {
                    id: id.to_string(),
                    message: Some(format!(
                        "Downloaded to {}",
                        completed.primary_path.display()
                    )),
                });
            }
            Err(DownloadError::Cancelled) => {
                ...
                self.event_emitter
                    .emit(DownloadEvent::DownloadCancelled { id: id.to_string() });
            }
            Err(e) => {
                tracing::warn!(id = %id, error = %e, "Download failed");
                let item = QueuedItem::new(id.clone());
                self.queue.write().await.mark_failed(item, e.to_string());
            }
        }
    }
...
impl DownloadManagerPort for DownloadManagerImpl {
    async fn queue_download(&self, request: DownloadRequest) -> Result<DownloadId, DownloadError> {
        ...
        let position = {
            let mut queue = self.queue.write().await;
            queue.queue_sharded(&id, shard_files, has_active)?
        };
