...
impl ModelRegistrarPort for ModelRegistrar {
    async fn register_model(&self, download: &CompletedDownload) -> Result<Model, RepositoryError> {
        ...
        let gguf_metadata = match self.gguf_parser.parse(file_path) {
        ...
        if let Some(ref meta) = gguf_metadata {
            model.tags = self.gguf_parser.detect_capabilities(&meta.metadata);
        }

        // For sharded models, add a note about the number of shards
        if download.is_sharded {
            let shard_count = download.all_paths.len();
...
mod tests {
    ...
    async fn test_register_sharded_model() {
        ...
        let model = result.unwrap();
        assert!(model.quantization.unwrap().contains("sharded: 4 parts"));
    }
