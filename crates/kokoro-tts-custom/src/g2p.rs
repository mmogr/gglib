...
/// Use misaki-rs (the correct G2P for Kokoro) when the `misaki-g2p` feature is
/// enabled. Falls through to the built-in espeak/cmudict G2P for Chinese text
/// and for v1.1 voices, since misaki-rs is English-only.
#[cfg(feature = "misaki-g2p")]
pub fn g2p(text: &str, use_v11: bool) -> Result<String, G2PError> {
    // v1.1 voices use Chinese phonemization — misaki-rs is English-only,
    // so delegate to the original implementation.
    if use_v11 {
        return g2p_builtin(text, use_v11);
    }

    // Check if the input contains any Chinese characters.
    // If so, fall through to the built-in G2P which handles mixed
    // Chinese/English text via jieba segmentation + pinyin.
    let has_chinese = text.chars().any(|c| ('\u{4E00}'..='\u{9FFF}').contains(&c));
    if has_chinese {
        return g2p_builtin(text, use_v11);
    }

    // Pure English text → use misaki-rs for accurate Kokoro-native phonemes.
    use std::sync::LazyLock;
    static MISAKI: LazyLock<misaki_rs::G2P> =
        LazyLock::new(|| misaki_rs::G2P::new(misaki_rs::Language::EnglishUS));

    let (phonemes, _tokens) = MISAKI
        .g2p(text)
        .map_err(|_e| G2PError::EnptyData)?;
    if phonemes.trim().is_empty() {
        return Err(G2PError::EnptyData);
    }

    log::debug!("misaki-rs phonemes: {}", phonemes);
    Ok(phonemes)
}

/// Original built-in G2P using espeak/cmudict — used as fallback for Chinese
/// text and v1.1 voices, and as the sole G2P when `misaki-g2p` is disabled.
#[cfg(not(feature = "misaki-g2p"))]
pub fn g2p(text: &str, use_v11: bool) -> Result<String, G2PError> {
    g2p_builtin(text, use_v11)
}

/// The original G2P implementation (espeak dict for English, pinyin for Chinese).
fn g2p_builtin(text: &str, use_v11: bool) -> Result<String, G2PError> {
