...
mod tests {
    ...
    use gglib_core::ports::{CatalogError, ModelLaunchSpec, ModelSummary, RunningTarget};
    ...
    impl ModelRuntimePort for MockRuntimePort {
        async fn ensure_model_running(
            ...
            if self.should_fail {
                ...
                Ok(RunningTarget::local(8080, 1, model_name.to_string(), 4096))
            }
        }
    ...
    impl ModelCatalogPort for MockCatalogPort {
        ...
        async fn resolve_for_launch(
            ...
            if name == "test-model" {
                Ok(Some(ModelLaunchSpec {
                    id: 1,
                    name: "test-model".to_string(),
                    file_path: std::path::PathBuf::from("/models/test-model.gguf"),
                    tags: vec!["test".to_string()],
                    architecture: Some("llama".to_string()),
