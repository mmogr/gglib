...
fn inject_streaming_body_overrides_sets_include_usage_and_return_progress() {
    ...
    let (out, client_wants_progress) = inject_streaming_body_overrides(body);
    ...
    assert!(
        ...
        "the override is the proxy's own; a client that never asked for \
         progress frames must not be reported as wanting them"
    );
}
...
fn inject_streaming_body_overrides_forces_include_usage_even_if_client_disabled_it() {
    ...
    let (out, _) = inject_streaming_body_overrides(body);
    let value: serde_json::Value = serde_json::from_slice(&out).expect("valid json");
    assert_eq!(
...
#[test]
fn inject_streaming_body_overrides_reports_a_client_that_asked_for_progress() {
    let body = Bytes::from(r#"{"model":"foo","messages":[],"return_progress":true}"#);
    let (out, client_wants_progress) = inject_streaming_body_overrides(body);
    let value: serde_json::Value = serde_json::from_slice(&out).expect("valid json");
    assert_eq!(value["return_progress"], true);
    assert!(
        client_wants_progress,
        "a client that asked for prompt_progress frames must still receive them"
    );
}

#[test]
fn inject_streaming_body_overrides_treats_explicit_false_as_not_asked() {
    let body = Bytes::from(r#"{"model":"foo","messages":[],"return_progress":false}"#);
    let (_, client_wants_progress) = inject_streaming_body_overrides(body);
    assert!(
        !client_wants_progress,
        "return_progress:false is a client declining the frames"
    );
}
...
fn inject_streaming_body_overrides_leaves_non_json_body_unchanged() {
    ...
    let (out, client_wants_progress) = inject_streaming_body_overrides(body.clone());
    assert_eq!(out, body, "non-JSON bodies must pass through unchanged");
    assert!(
        !client_wants_progress,
        "a body the proxy could not read never asked for anything"
    );
}
