...
//! |----------------------------------|-----------|
//! | `gglib-orchestrator`             | Auto mode — `HitlMode::None`, full pipeline, streams events as markdown. |
//! | `gglib-orchestrator:interactive` | Interactive mode — pauses at plan gate, embeds sentinel, resumes on next turn. |
//! | `gglib-orchestrator:native`      | Returns HTTP 400 directing client to `/api/orchestrator/run`. |
//!
...
pub const VIRTUAL_MODEL_AUTO: &str = "gglib-orchestrator";

/// Interactive mode: pauses at plan gate; sentinel-based resume.
pub const VIRTUAL_MODEL_INTERACTIVE: &str = "gglib-orchestrator:interactive";

/// Native mode: returns HTTP 400; instructs client to use `/api/orchestrator/run`.
pub const VIRTUAL_MODEL_NATIVE: &str = "gglib-orchestrator:native";
...
// =============================================================================
// OrchestratorRunnerPort — injected dependency
// =============================================================================
...
pub struct OrchestratorRunParams {
    ...
    pub orchestrator_repo: Option<Arc<dyn OrchestratorRepositoryPort>>,
...
///
/// Implemented by [`gglib_runtime::OrchestratorRunnerAdapter`].  The proxy
...
#[async_trait]
pub trait OrchestratorRunnerPort: Send + Sync + fmt::Debug {
    /// Execute the orchestrator for `goal`, streaming [`OrchestratorEvent`]s
    /// to `tx`.
    ///
...
/// [`execute`](gglib_agent::orchestrator::execute)) together with the
/// [`LlmCompletionPort`](gglib_core::ports::LlmCompletionPort) and
/// [`ToolExecutorPort`](gglib_core::ports::ToolExecutorPort) adapters.
///
/// `approval_registry` and `orchestrator_repo` are shared with the Axum
/// `POST /api/orchestrator/approve/:id` handler so that interactive-mode runs
/// are visible in `GET /api/orchestrator/runs`.
#[derive(Clone)]
pub struct OrchestratorDeps {
    /// Executes the orchestrator pipeline (planning → workers → synthesis).
    pub runner: Arc<dyn OrchestratorRunnerPort>,
    /// Resolves HITL approval gates.
    pub approval_registry: Arc<dyn OrchestratorApprovalRegistryPort>,
    /// Persists and retrieves orchestrator run records.
    pub orchestrator_repo: Arc<dyn OrchestratorRepositoryPort>,
}
...
fn handle_native_mode() -> Response {
    (
        StatusCode::BAD_REQUEST,
        axum::Json(ErrorResponse::new(
            "gglib-orchestrator:native requires the gglib API; \
             use POST /api/orchestrator/run directly.",
            "unsupported_model",
        )),
    )
        .into_response()
...
async fn handle_auto_mode(deps: &OrchestratorDeps, req: OrchestratorRequest) -> Response {
    ...
    let params = OrchestratorRunParams {
        hitl_mode: HitlMode::None,
        approval_registry: Some(Arc::clone(&deps.approval_registry)),
        orchestrator_repo: Some(Arc::clone(&deps.orchestrator_repo)),
...
async fn handle_interactive_mode(deps: &OrchestratorDeps, req: OrchestratorRequest) -> Response {
...
async fn resume_interactive_run(
    ...
    match intent {
        ...
        ApprovalIntent::Approve(edit) => {
            ...
            let run = match deps.orchestrator_repo.get_run(&run_id).await {
                Ok(Some(r)) => r,
                Ok(None) => {
                    return (
                        StatusCode::NOT_FOUND,
...
/// Build an SSE `Response` that maps [`OrchestratorEvent`]s to OpenAI chunks.
///
/// `interactive` — when `true`, the stream terminates on [`OrchestratorEvent::AwaitingApproval`]
...
fn build_sse_stream(
    ...
    let sse_stream = async_stream::stream! {
        ...
        while let Some(event) = inner.next().await {
            match &event {
                OrchestratorEvent::AwaitingApproval { approval_id, kind: _ } if interactive => {
                ...
                _ => {
                    ...
                    if matches!(event, OrchestratorEvent::OrchestratorComplete { .. }) {
                        yield stop_chunk(&stream_id, &model);
                        yield done_event();
                        return;
                    }
                    if matches!(event, OrchestratorEvent::OrchestratorError { .. }) {
                        yield stop_chunk(&stream_id, &model);
                        yield done_event();
                        return;
                    }
        ...
        // Channel closed without OrchestratorComplete (e.g. cancelled).
        yield stop_chunk(&stream_id, &model);
        yield done_event();
    };
...
// =============================================================================
// OrchestratorEvent → markdown content
// =============================================================================

/// Map a single [`OrchestratorEvent`] to markdown text for the SSE stream.
///
/// Returns `None` for events that should not produce visible output
/// (e.g. progress pings, internal bookkeeping).
pub(crate) fn orchestrator_event_to_content(event: &OrchestratorEvent) -> Option<String> {
    match event {
        ...
        OrchestratorEvent::NodeComplete { .. } => Some("\n\n".to_string()),
