...
pub(super) struct ExecutableSearcher<'a> {
    env: &'a dyn EnvProvider,
    fs: &'a dyn FsProvider,
}

impl<'a> ExecutableSearcher<'a> {
    pub(super) fn new(env: &'a dyn EnvProvider, fs: &'a dyn FsProvider) -> Self {
        Self { env, fs }
    }

    /// Search for a command in PATH environment variable.
    pub(super) fn search_in_path(&self, command: &str) -> Vec<Attempt> {
    ...
    #[cfg(target_os = "macos")]
    pub(super) fn search_in_etc_paths(&self, command: &str) -> Vec<Attempt> {
        let mut attempts = Vec::new();
    ...
    pub(super) fn search_platform_defaults(&self, command: &str) -> Vec<Attempt> {
        let mut attempts = Vec::new();
    ...
    pub(super) fn search_node_managers(&self, command: &str) -> Vec<Attempt> {
        let mut attempts = Vec::new();
    ...
    pub(super) fn search_user_paths(&self, command: &str, user_paths: &[String]) -> Vec<Attempt> {
        let mut attempts = Vec::new();
