...
#![allow(dead_code)] // Some error variants/methods not yet used by callers
...
impl McpManager {
    ...
    async fn start_stdio_server(
        ...
        let command = server.config.command.as_ref().ok_or_else(|| {
    ...
    pub async fn running_servers(&self) -> Vec<i64> {
        let servers = self.servers.read().await;
