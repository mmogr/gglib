...
use gglib_core::{
    AppEvent, AppEventEmitter, McpErrorInfo, McpServer, McpServerRepository,
...
impl McpService {
    ...
    pub async fn start_server(&self, id: i64) -> Result<Vec<McpTool>, McpServiceError> {
        ...
        let tools = self.manager.start_server(server).await.map_err(|e| {
            // Emit error event
            self.emitter.emit(AppEvent::mcp_server_error(
                McpErrorInfo::process(Some(id), &server_name, e.to_string()),
            ));
...
mod tests {
    ...
    use gglib_core::{AppEvent, McpEnvEntry, McpServerConfig, McpServerType, NoopEmitter};
    use std::sync::Mutex;
