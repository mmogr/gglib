...
/// # Why a singleton?
///
/// The `LOG_MANAGER` is an intentional singleton because:
///
/// 1. **Cross-process logging**: Log entries are generated by spawned llama-server
///    processes that outlive individual request handlers. A global singleton provides
///    consistent access from any context (process spawning, SSE streaming, HTTP handlers).
///
/// 2. **Broadcast semantics**: The internal `broadcast::Sender` requires a single source
///    of truth. Multiple managers would each have their own broadcast channel, making it
///    impossible to aggregate logs from all servers.
///
/// 3. **Lifetime independence**: Unlike database connections (which are per-request or
///    per-command), logs must persist across the entire application lifetime and be
///    accessible from detached async tasks.
///
/// This pattern follows Rust's "controlled global state" idiom using `OnceLock` for
/// safe lazy initialization. The `Arc<ServerLogManager>` ensures thread-safe sharing.
static LOG_MANAGER: std::sync::OnceLock<Arc<ServerLogManager>> = std::sync::OnceLock::new();

/// Get or initialize the global log manager.
///
/// Returns a clone of the `Arc<ServerLogManager>`, allowing shared access from any context.
