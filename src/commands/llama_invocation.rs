...
/// Resolved model information ready for command invocation.
#[derive(Debug, Clone)]
...
/// Resolve a model by identifier and validate it exists on disk.
///
/// This helper consolidates the repeated pattern of:
/// 1. Looking up model in database
/// 2. Extracting the model ID
...
pub async fn resolve_model_for_invocation(
    ...
    let model_id = model
        .id
        .ok_or_else(|| anyhow!("Model '{}' has no ID", identifier))?;
    ...
    Ok(ResolvedModel { model, model_id })
}
...
pub fn log_model_info(model: &Gguf, binary_name: &str) {
    println!("Starting {} with model: {}", binary_name, model.name);
    println!("Model path: {}", model.file_path.display());
}
