...
///
/// This is the only event type emitted by the download system.
...
///   | { type: "download_progress"; id: string; downloaded: number; total: number; speed_bps: number }
///   | { type: "shard_progress"; id: string; shard_index: number; total_shards: number; ... }
...
pub enum DownloadEvent {
    ...
    ///
    /// Sent on connection and after significant state changes.
    QueueSnapshot {
        /// All items currently in the queue (current + pending + failed).
        items: Vec<DownloadSummary>,
        /// Maximum queue capacity.
        max_size: u32,
    },

    /// A download has started (moved from queued to downloading).
    DownloadStarted {
    ...
    DownloadProgress {
        ...
        total: u64,
        /// Current download speed in bytes per second (EWA smoothed).
        speed_bps: f64,
    ...
    ///
    /// Contains both per-shard and aggregate progress.
    ShardProgress {
        /// Canonical ID of the download (group ID, not shard ID).
        id: String,
        ...
        /// Current download speed in bytes per second (EWA smoothed).
        speed_bps: f64,
        /// Estimated time remaining in seconds (based on aggregate).
        eta_seconds: f64,
...
impl DownloadEvent {
    ...
    pub fn queue_snapshot(items: Vec<DownloadSummary>, max_size: u32) -> Self {
...
mod tests {
    ...
    fn test_event_serialization() {
        let event = DownloadEvent::started("model:Q4_K_M");
        let json = event.to_json();
        assert!(json.contains(r#""type":"download_started""#));
        assert!(json.contains(r#""id":"model:Q4_K_M""#));
    }
    ...
    #[test]
    fn test_shard_progress_event() {
        let event = DownloadEvent::shard_progress(
            "model:Q4_K_M",
            1,
            3,
            "model-00002.gguf",
