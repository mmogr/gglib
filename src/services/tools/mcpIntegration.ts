...
import { McpService, McpTool } from '../mcp';
 ...
 */
function createMcpExecutor(serverId: string, toolName: string): ToolExecutor {
 ...
 */
export function getMcpSource(serverId: string): ToolSource {
  return `mcp:${serverId}`;
}
 ...
 */
export function registerMcpTools(serverId: string, tools: McpTool[]): number {
...
export async function syncAllMcpTools(): Promise<{ added: number; removed: number }> {
  ...
  try {
    ...
    for (const server of servers) {
      if (McpService.isRunning(server) && server.config.id) {
        const serverId = server.config.id.toString();
        added += registerMcpTools(serverId, server.tools);
      }
    }
  } catch (err) {
