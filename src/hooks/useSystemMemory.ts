...
import { fetchSystemMemoryInfo } from "../services/settings";
...
export function useSystemMemory(): UseSystemMemoryReturn {
  ...
  const loadMemoryInfo = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const info = await fetchSystemMemoryInfo();
      setMemoryInfo(info);
    } catch (err) {
  ...
  const availableMemory = memoryInfo
    ? (memoryInfo.gpu_memory_bytes ?? memoryInfo.total_ram_bytes)
    : null;
  ...
  const checkFit = useCallback(
    (fileSizeBytes: number): FitStatus => {
      if (!availableMemory) {
        // Default to "fits" if we can't determine - better UX than blocking
        return 'fits';
      }
  ...
  const getTooltip = useCallback(
    (fileSizeBytes: number): string => {
      ...
      const memoryType = memoryInfo.gpu_memory_bytes 
        ? (memoryInfo.is_apple_silicon ? "Unified Memory" : "GPU VRAM")
        : "System RAM";
