...
import { McpServerConfig, McpServerType, McpServerInfo } from "../services/mcp";
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      const config: Omit<McpServerConfig, "id"> = {
        ...
        env: envVars.filter(([key]) => key.trim()), // Remove empty keys
      };

      if (serverType === "stdio") {
        config.command = command.trim();
        config.args = args.trim() ? args.trim().split(/\s+/) : undefined;
    ...
    [name, serverType, command, args, cwd, url, envVars, autoStart, enabled, onSave, onClose]
  );
