...
Custom hooks for the chat messages panel: thread hydration, message deletion, council mode, live timer ticks, and AI title generation. They are called from the panel root so that state which touches the thread runtime stays in the component that owns it.
...
| `useCouncilMode.ts` | Council (orchestrator) toggle state, submit registration, and the run-start ref |
