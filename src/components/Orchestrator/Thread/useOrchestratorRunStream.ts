/**
 * useOrchestratorRunStream — SSE streaming wired into the OrchestratorRegistry.
 *
 * A registry-scoped counterpart to the legacy `useOrchestrator` hook.
 * Instead of pulling dispatch from the page-singleton OrchestratorContext,
 * it accepts `dispatch`, `setAbortController`, and `abort` as injected
 * dependencies from `useOrchestratorRun(runId)` — giving each run its own
 * isolated stream without any coupling to the singleton.
 *
 * Key differences from `useOrchestrator`:
 ...
 * `useOrchestrator` so that hook remains untouched (Phase 1 additive
 * constraint). It will be deduplicated in Phase 7 during the final cleanup.
 *
 * @module components/Orchestrator/Thread/useOrchestratorRunStream
 */
...
import type { OrchestratorAction, OrchestratorSession } from '../../../contexts/OrchestratorContext';
import type { OrchestratorEvent, ApprovalDecisionPayload } from '../../../types/orchestrator';
import { appLogger } from '../../../services/platform';

// ─── Event → Action translation ───────────────────────────────────────────────

/**
 * Map a raw SSE OrchestratorEvent to a typed OrchestratorAction for the
 * reducer, or return null for purely informational events.
 *
 * Duplicated from `hooks/useOrchestrator/useOrchestrator.ts` — see module
...
export interface UseOrchestratorRunStreamOptions {
  /** Dispatch bound to this run's registry slot (from useOrchestratorRun). */
   ...
   */
  getPendingApproval: () => OrchestratorSession['pendingApproval'];
 ...
 * into the registry so `useOrchestratorRun(runId)` subscribers receive them.
 *
 * Mount/unmount cleanup (aborting the stream) is the caller's responsibility
 * because this hook does not know its runId — the caller (OrchestratorThread)
 * handles `unregister` + `abort` in its own cleanup effect.
 */
export function useOrchestratorRunStream({
  dispatch,
  setAbortController,
  abort,
  getPendingApproval,
    ...
    async (goal: string, hitlMode?: string) => {
      ...
      try {
        ...
        if (err instanceof Error && err.name !== 'AbortError') {
          ...
          appLogger.error('hook', 'OrchestratorThread run failed', { error: err.message });
        }
      }
    },
  ...
  const approve = useCallback(
    async (payload: ApprovalDecisionPayload) => {
      ...
      try {
        ...
        appLogger.error('hook', 'OrchestratorThread approval failed', { error: message });
      }
    },
