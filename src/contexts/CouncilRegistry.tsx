/**
 * CouncilRegistry — per-run context isolation.
 *
 * Replaces the page-singleton CouncilProvider with a registry pattern.
 * A single CouncilRegistryProvider mounts at the app root and manages a
 * Map<runId, CouncilSession> so multiple CouncilThread instances
 ...
 *
 * 4. STABLE SENTINEL — useCouncilRun on an unregistered id returns a
 ...
 *    The legacy CouncilProvider and useCouncilContext remain intact.
 *
 * Exported hooks:
 *   useCouncilRun(runId)   — state + dispatch for a single run
 *   useCouncilRegistry()  — lifecycle: register / seed / promote / unregister
 *   useCouncilRunsList()  — global runs list, decoupled from per-run state
 *
 * Exported utilities:
 ...
 *
 * @module contexts/CouncilRegistry
 */
...
import type { CouncilRun } from '../types/council';
...
// createEmptySession is not exported from CouncilContext, so we
...
function createPerRunEmptySession(): CouncilSession {
  return {
    phase: 'idle',
    graph: null,
    nodeStates: {},
 ...
 */
const UNREGISTERED_SESSION: CouncilSession = Object.freeze(
 ...
 * CouncilRegistryProvider. It is never recreated on re-renders.
...
class CouncilRegistry {
   ...
   * Used by HistoricalCouncilThread after it replays persisted events.
   */
  seed(runId: string, session: CouncilSession): void {
   ...
   */
  getSession(runId: string): CouncilSession {
  ...
  setRunsList(runs: CouncilRun[], loading = false): void {
    this.runsList = runs;
...
const RegistryContext = createContext<CouncilRegistry | null>(null);

function useRegistry(): CouncilRegistry {
  const ctx = useContext(RegistryContext);
  if (ctx === null) {
    throw new Error(
      'CouncilRegistry hooks must be used within <CouncilRegistryProvider>.',
    );
  }
  return ctx;
}
...
/**
 * Mount once near the app root — alongside the existing CouncilProvider
 ...
 */
export function CouncilRegistryProvider({ children }: { children: ReactNode }) {
  const registryRef = useRef<CouncilRegistry | null>(null);
  if (registryRef.current === null) {
    registryRef.current = new CouncilRegistry();
  }
 ...
 */
export function useCouncilRun(runId: string): UseOrchestratorRunReturn {
 ...
 *
 * Typical usage:
 *   - CouncilThread: register on mount, unregister on unmount.
 *   - Streaming hook: promoteRun when the server returns the real id.
 *   - HistoricalCouncilThread: seed after replaying persisted events.
 */
export function useCouncilRegistry(): UseOrchestratorRegistryReturn {
  const registry = useRegistry();
  ...
  const seed = useCallback(
    (runId: string, session: CouncilSession) => registry.seed(runId, session),
    [registry],
  );
...
export interface UseOrchestratorRunsListReturn {
  runs: CouncilRun[];
  loading: boolean;
  setRuns: (runs: CouncilRun[], loading?: boolean) => void;
 ...
 * per-run reducer and does not cause any CouncilThread to re-render.
 *
 * Intended for the RunsList panel and chat history sidebar.
 */
export function useCouncilRunsList(): UseOrchestratorRunsListReturn {
  const registry = useRegistry();
  ...
  const setRuns = useCallback(
    (r: CouncilRun[], l?: boolean) => registry.setRunsList(r, l),
    [registry],
  );
