...
fn test_validate_various_extensions() {
    ...
    for (filename, should_be_valid) in test_cases {
        ...
        let result = validation::validate_file(file_path.to_str().unwrap());
        if should_be_valid {
            assert!(result.is_ok(), "File {} should be valid", filename);
        } else {
            assert!(result.is_err(), "File {} should be invalid", filename);
        }
    }
}
...
fn test_validate_and_parse_gguf_with_nonexistent_file() {
    let result = validation::validate_and_parse_gguf("/nonexistent/file.gguf");
    assert!(result.is_err());
...
fn test_validate_and_parse_gguf_with_wrong_extension() {
    ...
    let result = validation::validate_and_parse_gguf(file_path.to_str().unwrap());
    assert!(result.is_err());
...
fn test_create_mock_gguf_file_for_parsing() {
    ...
    let parse_result = validation::validate_and_parse_gguf(file_path.to_str().unwrap());
