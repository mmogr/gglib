...
use gglib_db::ensure_column_exists;
...
async fn test_ensure_column_exists_adds_new_column() {
    ...
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM pragma_table_info('models') WHERE name = 'test_column'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
...
async fn test_ensure_column_exists_accepts_valid_types() {
    ...
    assert!(
        ensure_column_exists(&pool, "models", "col_text", "TEXT")
            .await
            .is_ok()
    );
    assert!(
        ensure_column_exists(&pool, "models", "col_int", "INTEGER")
            .await
            .is_ok()
    );
    assert!(
        ensure_column_exists(&pool, "models", "col_real", "REAL")
            .await
            .is_ok()
    );
    assert!(
        ensure_column_exists(&pool, "models", "col_blob", "BLOB")
            .await
            .is_ok()
    );
}
